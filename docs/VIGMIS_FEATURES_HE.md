...
- זיהוי שפה אוטומטי לפי טווחי Unicode (עברית / ערבית / רוסית / אנגלית) — נשמר כ-fallback בלבד. ראו סעיף 34.5 לשפת תוכן מפורשת.
...
### 3.1 Google Ads
- חיבור דרך Google OAuth — **נתיב נפרד מ-GA4** (עדכון 2026-06-03).
- Scope: `adwords` בלבד. `/auth/google` → Google Ads.
- לאחר חיבור: בחירת חשבון (GET `/connectors/google/accounts`, POST `/connectors/google/select-account`).
...
### 3.3 TikTok
- שני APIs נפרדים:
  - **TikTok for Developers** (open.tiktokapis.com) — Content Posting API. אושר. Scopes: `user.info.basic`, `video.upload`. הערה: `video.publish` הוסר כי לא אושר — מוגש מחדש נפרד.
...
- חיבור דרך Google OAuth — **נתיב נפרד מ-Google Ads** (עדכון 2026-06-03).
- Scope: `analytics.readonly` בלבד. `/auth/google/analytics` → GA4 בלבד.
...
## 15. תמחור, מסלולים ומכסות — הוכרע סופית (2026-06-03)

מקור האמת בקוד: `apps/api/src/billing/pricing.ts`. דף המחירון הציבורי: `/pricing`.

**עיקרון:** Vigmis מוכרת ניהול פרסום. מכסות גדלות עם התקציב. מרווח גולמי ~80%+ בכל מדרגה. אין מגבלת קמפיינים ללקוח.

### 15.1 מסלולים

| | Grow (code: free) | Scale (code: pro) |
|---|---|---|
| Monthly subscription | $0 | $29 |
| Management fee | 7% of ad spend | 6% of ad spend |
| Minimum commission (floor) | $29 | $29 |
| **Minimum total / month** | **$29** | **$58** |
| Optimization | 3x / day | 6x / day |
| Briefing | Weekly | Daily |
| Social posts included / month | — | 5 (either type) |
| AI reply drafts included / month | 100 | 300 |
| Video included / month | — | 1 (any type) |
| Image Creatives included / month | — | 3 |
| Users per workspace | 1 | Up to 3 |

**שמות קוד:** `'free'` = Grow, `'pro'` = Scale. אל לשנות ערכי DB.

Scale זול יותר מ-Grow כשפרסום עולה על ~$500/חודש.

### 15.2 תוספות בתשלום (Metered add-ons)

| Item | Price | Notes |
|---|---|---|
| Social post — with new AI image | $1.00 | AI caption + new generated image + publish to FB/IG |
| Social post — with own/reused creative | $0.70 | AI caption + user's uploaded or library image + publish |
| AI reply draft (after bundle) | $0.05 | After free monthly bundle |
| Extra 25 AI Strategy Sessions | $9.00 | Add-on pack |
| Image Creative | $5.00 | Standalone ad image, not tied to a post |
| Animation video | $8.00 | Publish to FB / IG / TikTok included |
| Cinematic video | $12.00 | Publish to FB / IG / TikTok included |
| Avatar video (AI spokesperson) | $15.00 | Publish to FB / IG / TikTok included |

**חשוב:**
- פוסט = כיתוב AI + תמונה + פרסום. $0.30 הנחה אם הלקוח מביא תמונה משלו או ממאגר.
- Image Creative = נכס עצמאי לפרסום בתשלום (paid ads), לא פוסט.
- TikTok אינו מוצר נפרד — קונים סרטון (כל סוג), מפרסמים בכל פלטפורמה ללא עלות נוספת.
- סרטון שנרכש = פרסום ב-FB / IG / TikTok כלול ללא הגבלת כמות פרסומים.
- Scale: 5 פוסטים חינם חלים על שני הסוגים. 1 וידאו חינם = כל סוג (גם הכי יקר).

### 15.3 מכסות AI לפי מדרגת תקציב

| Tier | Monthly ad spend | AI Strategy Sessions | Comments auto-handled | Shopify products |
|---|---|---|---|---|
| T1 | up to $1,000 | 30 | 300 | 500 |
| T2 | $1,001 - $3,000 | 75 | 800 | 1,500 |
| T3 | $3,001 - $6,000 | 150 | 2,000 | 5,000 |
| T4 | $6,001 - $12,000 | 300 | 4,000 | 10,000 |
| T5 | $12,000+ | 400+ | 6,000+ | Unlimited |

AI Strategy Session = סשן צ'אט אחד עד 12 הודעות. מכסה נגמרת = הודעה רכה, לא שבירה. חבילה נוספת: 25 sessions ב-$9.

### 15.4 מפסק ביטחון (Circuit Breaker / Fair Use)

| Trigger | Action |
|---|---|
| AI cost reaches 25% of monthly fee | Degrade: routine tasks to cheap model, pause non-essential crons |
| AI cost reaches 40% of monthly fee | Freeze: all AI paused until 1st of next month + ops alert |
| Session quota reached | Soft block: buy pack ($9) or wait for reset |
| Reply bundle exhausted | $0.05 per additional reply |
| Abnormal burst (10x normal in a day) | Rate-limit + flag |

גילוי מלא ללקוח בדף `/pricing` בסקשן "Complete billing disclosure" (7 פסקאות).

### 15.5 הוכחת רווחיות

| Customer | Fee | AI cost | Infra | Left | Margin |
|---|---|---|---|---|---|
| $300 Grow (floor) | $29 | $9.90 | $1.94 | $17.16 | 59% |
| $1,000 Grow | $70 | $12.90 | $3.13 | $53.97 | 77% |
| $5,000 Grow | $350 | $52.00 | $11.25 | $286.75 | 82% |
| $500 Scale | $59 | ~$8 | ~$2 | ~$49 | 83% |
| $3,000 Scale | $209 | ~$30 | ~$8 | ~$171 | 82% |

### 15.6 מודל AI

`packages/ai-router/src/config.ts` — החלטות: Claude Sonnet 4.6; תוכן: GPT-4o; סיווג: gpt-4o-mini; תמונות: DALL-E 3. לא לשנות מודלים לאלה יקרים יותר בלי חישוב מחדש.

### 15.7 סליקה

- **כרגע:** אין סליקה פעילה. `/pricing` = תצוגה בלבד.
- **שלב 1:** Meshulam (ישראלי).
- **שלב 2:** חברת בת + Stripe.
- **לא בדרך:** Stripe Direct, Paddle, Lemon Squeezy, 2Checkout.

### 15.8 Multi-user (Team Members) — נבנה (2026-06-03)

- migration 041 על Supabase — team_members + team_invites.
- Auth middleware: own tenant -> team_members -> create new.
- API: GET/POST/DELETE /team, POST /team/invite, POST /team/accept.
- UI: /settings/team, /join?token=xxx.
- Grow = 1 seat, Scale = 3 seats. Token בתוקף 7 ימים, שליחה ב-SendGrid.

### 15.9 סטטוס מימוש

- pricing.ts — source of truth: Grow/Scale, מחירים, מכסות, add-ons, circuit breaker.
- calculator.ts — חישוב עמלה עם רצפה.
- services/usage.ts + migration 039 — אכיפת מכסות חיה.
- /pricing page — ציבורי, גילוי מלא, FAQ + reuse questions, add-ons.
- BillingClient — plan names = Grow / Scale.
- Multi-user — migration 041 רץ, auth, API, UI.
- migrations 023-041 — כולם רצו על Supabase.
...
- **TikTok Marketing API** — Standard Access ממתין לאישור. TikTok `video.publish` scope הוסר ממנויים הנוכחיים — הוגש מחדש בנפרד.
- **Google Ads Standard Access** — יוגש כשנגיע ל‑10,000 משתמשים.
- **Billing** — Meshulam טרם מומש.
- **Railway auto-deploy** — שינויים ב-API דורשים `railway up` ידני. תיעוד: `docs/RAILWAY_AUTODEPLOY.md`. אין auto-deploy מ-git push כרגע.
...
## 31. Multi-user — Team Members ✅ נבנה (2026-06-03)

### 31.1 מסד נתונים
- **migration 041** — שתי טבלאות חדשות:
  - `team_members` — שיוך משתמש ל-tenant (role: owner/member).
  - `team_invites` — הזמנות ממתינות עם token, email, expiry (7 ימים), SendGrid.

### 31.2 הרשאות לפי מסלול
| | Grow | Scale |
|---|---|---|
| Users per workspace | 1 | Up to 3 |

### 31.3 Auth Middleware
- בסיגנאפ/כניסה: Auth middleware בודק `team_members` → אם משתמש הוא חבר צוות, הוא מקבל את ה-`tenant_id` של הבעלים.
- סדר הפתרון: own tenant → team_members → create new.

### 31.4 API Routes
| Method | Route | תיאור |
|---|---|---|
| GET | `/team` | רשימת חברי הצוות הנוכחיים |
| POST | `/team/invite` | שליחת הזמנה לאימייל |
| DELETE | `/team/:userId` | הסרת חבר |
| POST | `/team/accept` | קבלת הזמנה עם token |

### 31.5 UI Pages
- `/settings/team` — ניהול צוות: רשימת חברים, הזמנה חדשה לפי אימייל, הסרה.
- `/join?token=xxx` — דף קבלת הזמנה: מציג שם workspace + כפתור "Accept Invitation". מנחה להרשמה אם אין חשבון.

---

## 32. Platform Connections — עדכונים (2026-06-03)

### 32.1 Google Ads ו-GA4 — OAuth נפרד
- `/auth/google` → Google Ads בלבד (scope: `adwords`).
- `/auth/google/analytics` → GA4 בלבד (scope: `analytics.readonly`).
- לאחר חיבור Google Ads: בחירת חשבון מפורשת — **Account Selector**:
  - `GET /connectors/google/accounts` — מחזיר רשימת חשבונות Ads.
  - `POST /connectors/google/select-account` — שמירת הבחירה ב-DB.

### 32.2 TikTok — scope מעודכן
- Scopes פעילים: `user.info.basic`, `video.upload`.
- `video.publish` **הוסר** (לא אושר) — הוגש מחדש בנפרד. Direct Post ימתין לאישור.

### 32.3 Onboarding — כל שלושת החיבורים לפני שאלון
- דף Onboarding מציג כרטיסי חיבור לשלוש פלטפורמות (Google Ads, Meta, TikTok) **לפני** השאלון העסקי.
- לקוח יכול לדלג, אבל ה-UI מציג בצורה ברורה שחיבור = תוצאות טובות יותר.

---

## 33. Settings ✅ נבנה (2026-06-03)

### 33.1 /settings/general — Brand Settings
- **לוגו עסקי:** העלאת תמונה לבחירה → נשמרת ב-Supabase Storage, bucket `logos`.
- `logo_url` נכתב ל-`client_settings.logo_url`.
- הלוגו מוזרק לכל תוכן שנוצר: פוסטים, תמונות, סרטונים.

### 33.2 /settings/team
- ניהול חברי צוות (ראו סעיף 31).

### 33.3 Migrations נוספות
| Migration | תוכן |
|---|---|
| 042 | עמודות `logo_url`, `content_language`, `ui_language` ב-`client_settings` |
| 043 | יצירת Supabase Storage bucket `logos` |

---

## 34. i18n — תמיכה רב-לשונית ✅ נבנה (2026-06-03)

### 34.1 שפות נתמכות — 10 שפות
| Code | שפה |
|---|---|
| `en` | English |
| `he` | Hebrew |
...
### 34.2 קבצי תרגום
- נמצאים ב-`apps/web/messages/<code>.json`.
- כל שפה — קובץ JSON עם כל מחרוזות ה-UI.

### 34.3 RTL
- עברית (`he`) וערבית (`ar`) מגדירות `html dir="rtl"` אוטומטית.

### 34.4 Language Selector
- קומפוננט בחירת שפה ב-nav ובדשבורד header.
- בחירה שומרת ב-session וב-`client_settings.ui_language`.

### 34.5 שפת תוכן
- `client_settings.content_language` — שפת תוכן מועדפת ליצירת פוסטים/קמפיינים.
- ערך `'auto'` = זיהוי מהאתר (Unicode heuristics כמקודם).
- ערך מפורש (`'he'`, `'en'`, וכו') = Vigmis תמיד מייצרת תוכן בשפה זו, ללא ניחוש.
- **מחליף** את מנגנון הזיהוי ההיוריסטי הישן.

---

## 35. Creative Brief & Content Pipeline ✅ נבנה (2026-06-03)

### 35.1 Brief Dialog
- נפתח לפני כל יצירת תוכן: פוסטים, סרטונים, תמונות.
- 4 שדות אופציונליים:
  | שדה | תיאור |
  |---|---|
  | Product | המוצר/שירות הנדון |
  | Message | המסר המרכזי |
  | Style | סגנון: professional / funny / urgent / inspirational |
  | CTA | קריאה לפעולה (מוגדר ברירת מחדל = `website_url`) |
- נתוני ה-brief מועברים לכל ה-generation APIs.

### 35.2 לוגו בתוכן
- לוגו מ-`client_settings.logo_url` מוזרק אוטומטית לתמונות וסרטונים שנוצרים.

### 35.3 CTA אוניברסלי
- `website_url` של הלקוח מתווסף כ-CTA לכל prompt של פוסט/תמונה/סרטון.

### 35.4 Preview לפני חיוב
- עבודות וידאו (Video Jobs) מציגות preview לפני חיוב.
- 3 כפתורי פעולה: **Approve** (חיוב + פרסום), **Request Revision**, **Cancel**.

### 35.5 Brief Context בפוסטים
- post generation מקבל brief block (product + message + style + CTA) לפני brand voice block.

---

## 36. Competitive Intelligence ✅ נבנה (2026-06-03)

שלושה כלי בינת-תחרות חדשים — Batch 8.

### 36.1 G1: Pre-launch Creative Scoring
- **API:** `POST /creatives/score`
- קלט: URL של תמונה (creative).
- עיבוד: GPT-4o Vision מנתח 4 ממדים.
- פלט:

| Dimension | תיאור |
|---|---|
| `attention` | עד כמה מושך תשומת לב מיידית |
| `clarity` | בהירות המסר |
| `emotion` | עוצמה רגשית |
| `cta_presence` | נוכחות ובהירות קריאה לפעולה |

- **ציון כולל:** 0–100.
- **Verdict:** `excellent` / `good` / `fair` / `poor` + טיפים לשיפור.
- **UI:** כפתור "Score this creative" על jobs שהושלמו ב-CreativeTab.

### 36.2 G2: Cross-Creative Theme Learning
- **API:** `GET /intelligence/creative-themes`
- ניתוח 90 הימים האחרונים של פוסטים ביצועים.
- מזהה אילו themes ויזואליים/קופי עובדים הכי טוב.
- **UI:** פאנל "Creative Themes" בלשונית Intelligence.

### 36.3 G3: Budget Scenario Modeling
- **API:** `GET /analytics/budget-forecast?budget=N`
- מחזיר 4 תרחישי תקציב:

| Scenario | Multiplier |
|---|---|
| Conservative | 50% of current |
| Current | baseline |
| Growth | 2× current |
| Aggressive | 4× current |

- מבוסס על נתוני GA4 ROAS היסטוריים.
- **UI:** כרטיס "Budget Scenarios" בלשונית Strategy עם input אינטראקטיבי.
