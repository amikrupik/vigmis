...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${apiUrl}/compliance/cron/recompute-trust`, {
      method: 'POST',
      headers: { 'x-cron-secret': cronSecret, 'Content-Type': 'application/json' },
    });
    if (!res.ok) return Response.json({ error: 'trust-recompute cron failed', details: await res.text() }, { status: 500 });
