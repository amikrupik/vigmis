...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${apiUrl}/compliance/cron/reattestation`, {
      method: 'POST',
      body: '{}',
      headers: { 'x-cron-secret': cronSecret, 'Content-Type': 'application/json' },
    });
    if (!res.ok) return Response.json({ error: 'reattestation cron failed', details: await res.text() }, { status: 500 });
